...
const TOOL_ICONS: Record<string, React.ReactNode> = {
  select: <MousePointer className="w-5 h-5 stroke-[1.9]" />,
  hand: <Hand className="w-5 h-5 stroke-[1.9]" />,
  pencil: <Pencil className="w-5 h-5 stroke-[1.9]" />,
  highlighter: <Highlighter className="w-5 h-5 stroke-[1.9]" />,
  rectangle: <Square className="w-5 h-5 stroke-[1.9]" />,
  circle: <Circle className="w-5 h-5 stroke-[1.9]" />,
  line: <Minus className="w-5 h-5 stroke-[1.9]" />,
  arrow: <ArrowUpRight className="w-5 h-5 stroke-[1.9]" />,
  text: <Type className="w-5 h-5 stroke-[1.9]" />,
  image: <ImageIcon className="w-5 h-5 stroke-[1.9]" />,
  laser: <Zap className="w-5 h-5 stroke-[1.9]" />,
  eraser: <Eraser className="w-5 h-5 stroke-[1.9]" />,
};
  ...
  return (
    <section id="tools" className="py-20 px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto relative z-20 select-none">
      ...
      <div className="text-center max-w-2xl mx-auto mb-14 space-y-3">
        <div className="inline-flex items-center gap-1.5 px-3.5 py-1 rounded-full bg-white/70 border border-[#27221e]/10 shadow-[0_2px_10px_rgba(0,0,0,0.03)] backdrop-blur-md">
        ...
        <p className="text-xs sm:text-sm text-[#6e6054] max-w-lg mx-auto leading-relaxed font-normal">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-7 items-stretch">
        {/* Left Side: 12-Tool Symmetrical 2x6 Button Grid with Clean Floating Icons */}
            ...
            {SUPPORTED_TOOLS.map((tool) => {
              ...
              return (
                <button
                  ...
                  className={`group relative p-3 rounded-2xl border transition-all duration-200 text-left flex flex-col justify-between min-h-[74px] cursor-pointer ${
                    isSelected
                      ? "bg-white text-[#27221e] shadow-[0_4px_16px_rgba(0,0,0,0.06)] border-[#c45a2c]/40 scale-[1.02] ring-1 ring-[#c45a2c]/30"
                      : "bg-[#faf8f5]/80 hover:bg-white text-[#5c4e43] border-[#27221e]/6 hover:scale-[1.01]"
                  }`}
                >
                  <div className="flex items-center justify-between w-full mb-1">
                    {/* Clean Floating Icon without bulky solid box */}
                    <div
                      className="transition-transform duration-200 group-hover:scale-110"
                      style={{ color: isSelected ? meta.color : "#6e6054" }}
                    >
                      {TOOL_ICONS[tool.id]}
                    </div>

                    <span className="text-[10px] font-mono font-medium px-1.5 py-0.5 rounded-md bg-black/[0.04] text-[#78695d]">
                      {tool.shortcut}
                    </span>
                  </div>
        ...
        {/* Right Side: Detail Card with Clean Floating Icon & Rounded Arrow */}
        <div className="lg:col-span-6 rounded-[32px] p-6 sm:p-8 bg-[#faf8f5]/95 border border-[#27221e]/8 shadow-[0_10px_35px_rgba(0,0,0,0.04)] flex flex-col justify-between relative overflow-hidden">
          <div className="space-y-5 relative z-10">
            {/* Tool Header with Clean Floating Icon */}
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div
                  className="transition-transform duration-300 hover:rotate-12"
                  style={{ color: funMeta.color }}
                >
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xl select-none">{funMeta.emoji}</span>
                  </div>
                  <p className="text-xs font-mono font-medium text-[#8a5d3b]">ToolType: &apos;{currentConfig.id}&apos;</p>
                </div>
              </div>
            ...
            {/* Description */}
            <p className="text-xs sm:text-sm text-[#64564c] leading-relaxed font-normal">
            ...
            <div className="p-3.5 rounded-2xl bg-white border border-[#27221e]/8 flex items-center justify-between gap-3 shadow-xs">
              <div className="space-y-0.5">
                <span className="text-[10px] font-mono text-[#8c7b6f] uppercase tracking-wider block font-medium">Pro-Tip &amp; Behavior</span>
            ...
            <div className="bg-white rounded-2xl p-4 border border-[#27221e]/8 font-mono text-xs text-[#44382f] space-y-1 shadow-xs">
              <div className="flex items-center justify-between text-[#8c7b6f] text-[10px] pb-1.5 border-b border-black/5">
          ...
          <div className="pt-4 border-t border-[#27221e]/6 flex flex-wrap items-center justify-between text-[11px] text-[#736357] gap-2 mt-4 relative z-10">
